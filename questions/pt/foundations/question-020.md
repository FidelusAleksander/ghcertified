---
question: "Os workflows do GitHub Actions são acionados por eventos. Quais destes são eventos válidos que o GitHub Actions suporta?"
documentation: "https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#about-events-that-trigger-workflows"
---

- [x] Um commit é enviado (push) para um branch
- [x] Um pull request é aberto
- [ ] Uma alteração é feita nas configurações do repositório
- [ ] Uma nova vulnerabilidade foi detectada em uma dependência  
> Este não é um evento válido que poderia acionar um workflow. É um recurso do Dependabot.
- [ ] Um novo membro foi adicionado ao repositório
