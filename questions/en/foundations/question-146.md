---
question: "What’s the difference between Git and GitHub?"
documentation: "https://learn.microsoft.com/en-us/training/modules/intro-to-git/1-what-is-vc"
---

- [x] Git lets you work with one or more local branches and push changes to a remote repository. GitHub acts as the remote repository, which is accessed through a website or command-line tools.
- [ ] Git is a distributed version control system (DVCS) that runs in the cloud. GitHub is an interface layer that provides access to Git technology through web browsers.
- [ ] Git is used by an individual contributor for local tracking. GitHub is used by multiple contributors to simplify group development work and collaboration.