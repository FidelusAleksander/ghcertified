---
question: "Your team is preparing to deploy a new feature using GitHub branches. What should be done to ensure the feature branch is ready for deployment?"
documentation: "https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/about-pull-requests"
---

- [ ] Commit all changes directly to the main branch without any code review or approval process.
- [ ] Delete the feature branch to finalize the deployment process without merging the changes.
- [x] Ensure all changes are reviewed and approved through a pull request before merging into the main branch.
