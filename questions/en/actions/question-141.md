...
- [ ] `hashFiles` is required for compatability with Windows runners.
- [ ] When using `hashFiles` as part of a cache key, if there is a cache miss, `hashFiles` gives additional debug info.  
- [ ] When using `hashFiles` as part of a cache key, then an additional step will be generated in the caller workflow. This workflow step prints the value of the SHA-256 hash of the cache key for reference purposes.  
