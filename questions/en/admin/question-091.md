...
- [ ] Direct integration with 3rd party vaults is supported natively within GitHub Actions without needing any additional configuration or authentication steps.
- [ ] Third party vaults should be accessed by embedding their API keys directly within the workflow files and using the API or CLI of that vendor to fetch secrets.
- [ ] GitHub Actions does not support the use of 3rd party vaults for secret management; all secrets must be stored exclusively within GitHub's built-in secrets.
