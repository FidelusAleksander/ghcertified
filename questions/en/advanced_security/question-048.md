---
question: "How can users exclude specific directories from secret scanning alerts on GitHub?"
---

> https://docs.github.com/en/code-security/secret-scanning/configuring-secret-scanning-for-your-repositories#excluding-directories-from-secret-scanning-alerts-for-users
- [x] By configuring a `secret_scanning.yml` file, under the `.github` path in the repository.
- [ ] Through the repository's `Security` tab, in the `Secret scanning` menu.
- [ ] Through the repository's `Settings` tab, in the `Code security and analysis` menu.
- [ ] By editing the repository's `README.md` file.
